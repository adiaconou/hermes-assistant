...
import { listEmails, getEmail as getEmailService, getThread as getThreadService } from '../services/google/gmail.js';
...
Combine operators: "from:hotel newer_than:1y subject:confirmation"`,
  ...
  handler: async (input, context) => {
    ...
    try {
      // Build query: use user query if provided, otherwise default to inbox
      let query = userQuery;
      if (!query) {
        query = include_spam ? undefined : 'is:inbox';
      } else if (!include_spam && !query.includes('in:spam')) {
        // Add inbox filter unless user explicitly searches spam
